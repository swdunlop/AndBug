...
(function() {
  ...
  value_view = function(ref, data) {
    var $doc = $('<span class="val">').text(data);
      $doc.data('ref', ref).data('data', data);
      $doc.click(function(){
          $(this).prop('contenteditable', true).css('color', 'black').css('backgroundColor', 'white');
      }).blur(function(){
          var text = $doc.text()
          if (text != $doc.data('data'))
          {
              $doc.css('color', 'red');
              var ref = $doc.data('ref').split('/');
              $.get('/e/' + ref.slice(2,5).join('/') + '/' + text + (ref.length >= 6 ? '/' + ref.slice(5).join('/') : ''))
                  .success(function(data){
                  if (data == 'True')
                  {
                      $doc.css('color', 'green').data('data', text);
                  }
                  else {
                      $doc.css('color', 'red').text($doc.data('data'));
                      alert(data);
                  }
              }).complete(function(){
                  $doc.prop('contenteditable', false);
                  setTimeout(function(){$doc.css({color:'', backgroundColor:''})}, 3000);
              });
          } else {
              $doc.prop('contenteditable', false);
